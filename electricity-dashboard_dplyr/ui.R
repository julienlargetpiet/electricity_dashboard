      ...
      page_sidebar(
        ...
        sidebar = tags$div(
          ...
          numericInput(
            ...
            value = 10,
            min = 1,
            max = length(unique(elec_data_pivoted$country)),
            width = "100%"
          )
        ),
        plotOutput(outputId = "comp_bar", height = "auto")
      )
    ),
    nav_panel(
      title = "About",
      tags$br(),
      p("*world: The mean, median and max are calculated according to the countries whose data are fully filled for the current year")
    ),
  )
)
