cons_bar1 <- function(country, source_energy){
  if (!(any(is.na(c(match(x = country, table = elec_data$country), 
    ...
    data <- data.frame(
                  "twh" = c(data[match(x = cur_yr, table = data$year), 
                            elec_var_vec[source_energy]],
                            data2[match(x = cur_yr, table = data2$year), 
                                  elec_cons_var_vec[source_energy]]
                         ),
                  "status" = c("production", "consumption on production")
                )
    print(data)
    ggplot(data = data, mapping = aes(y = reorder(status, twh), fill = status, x = twh)) + 
      geom_col() + 
